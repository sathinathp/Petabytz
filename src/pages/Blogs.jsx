...
export default function Blogs() {
  ...
  return (
    <div className="bg-white text-slate-800">
      ...
      {/* Blog Cards Grid */}
      <section className="py-14 sm:py-20 bg-[#FAF7F2]/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xl sm:text-2xl font-black text-[#08101E]">
              {selectedCategory === 'All' ? 'All Published Articles' : `${selectedCategory} Articles`}
              <span className="text-slate-400 text-sm font-normal ml-3">({filteredBlogs.length})</span>
            </h3>
          </div>
          ...
          {filteredBlogs.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
              ...
              <p className="text-slate-600 font-medium">No articles matched your search query.</p>
              <button
                onClick={() => { setSelectedCategory('All'); setSearchQuery(''); }}
                className="mt-4 px-4 py-2 rounded-lg bg-[#08101E] text-white text-xs font-bold hover:bg-[#E58A1F] transition"
              >
                Reset Filters
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredBlogs.map((blog) => (
                <div
                  ...
                  className="group cursor-pointer rounded-2xl overflow-hidden bg-white border border-slate-200/90 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
                >
                  {/* Card Cover Image with aspect ratio */}
                  <div className="relative aspect-[16/10] overflow-hidden bg-slate-900">
                    ...
                    <div className="absolute top-3 left-3">
                      <span className="px-2.5 py-1 rounded-md bg-white/90 backdrop-blur-xs text-[10px] font-bold text-[#08101E] uppercase tracking-wider font-mono">
                        {blog.industry || blog.category}
                      </span>
                    </div>

                    <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[11px] text-slate-200 font-medium font-mono">
                      <span>{blog.date}</span>
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    <div className="space-y-2.5">
                      <div className="text-[11px] font-bold uppercase tracking-wider text-[#E58A1F] font-mono">
                        {blog.badge}
                      </div>
                      <h4 className="text-base sm:text-lg font-bold text-[#08101E] group-hover:text-[#E58A1F] transition-colors leading-snug">
                        {blog.title}
                      </h4>
                    ...
                    <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <div className="w-8 h-8 rounded-full bg-slate-100 group-hover:bg-[#E58A1F] group-hover:text-white text-slate-700 flex items-center justify-center transition-all duration-200">
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
              <div className="flex items-center space-x-2 text-xs text-slate-500 font-mono">
                <span className="font-bold text-[#E58A1F] uppercase">{activeBlogModal.badge}</span>
                <span>•</span>
